...
/**
 * Operator that return true if the left parameter is lower than the right parameter, false if it is not.
...
@OperatorFindByRegex("<")
public class LowerThanOperator extends CompareOperator {

    @Override
    public CommandFlow onOperate(Module module, CommandFlow left, CommandFlow center, CommandFlow right) throws CommandException {
        return super.applyOperation(left, center, right, "<", this::lowerThan);
    }

    private boolean lowerThan(double a, double b) {
        return a < b;
    }
}
